...
function comment( value, options, cb ) {
	...
	var comment_on_issue = opt( options, 'comment_on_issue', false);

	var pull_request_id = process.env.TRAVIS_PULL_REQUEST;

	var commentLocation = (comment_on_issue ? '/issues/' : '/commits/')
	var commentLocationIdentifier = (comment_on_issue ? pull_request_id : commit_hash);
